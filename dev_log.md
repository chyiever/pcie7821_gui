...
## 2026-06-06 Space-Time 颜色条显示优化

### 问题与参考实现

根据现场反馈，Space-Time 图颜色条存在宽度不足、数据直方图区域拥挤以及数据分布显示不全的问题。本次读取并比较了参考项目 `E:\codes\das_fs_7825\pcie7825_gui` 中的 `src/time_space_plot.py`。参考项目使用更宽的 `HistogramLUTWidget`，并通过即时和延迟两次样式设置保证白色背景、黑色刻度轴和 gradient 字体稳定显示。

当前项目颜色条原先固定为 `90 px`，并在应用前面板 `vmin/vmax` 时把 Histogram 可视范围同样强制限制为 `vmin/vmax`。该处理虽然保证了颜色 levels 不自动变化，但会裁掉所有超出颜色范围的数据直方图分布，使用户无法观察实际数据是否超出颜色映射范围。

### 优化内容

颜色条宽度调整为最小 `140 px`、最大 `180 px`，主图与颜色条布局间距显式设置为 `6 px`。新增统一的 Histogram 样式方法，立即设置一次并在约 $100\ \mathrm{ms}$ 后再次设置，确保白色背景、Times New Roman 刻度字体、黑色坐标轴和 gradient 字体在 PyQtGraph 子组件初始化完成后仍然生效。

颜色映射 levels 与直方图显示范围已经分离。图像仍使用 `autoLevels=False`，颜色 levels 继续由前面板 `vmin/vmax` 唯一控制，Histogram region 继续禁止拖动。直方图显示范围改为根据当前显示数据中的有限最小值和最大值计算，并至少覆盖 `vmin/vmax`，同时增加 `5%` padding。这样可以显示完整数据分布，又不会重新引入颜色范围自动变化问题。

### 文档与验证

新增 `docs/2026-06-06-Space-Time颜色条显示优化.md`，详细记录当前颜色条问题、参考项目差异、优化方案、更新流程和现场验证重点。本次修改只影响颜色条布局、样式和直方图可视范围，不改变 Space-Time 数据缓冲、降采样、坐标映射、采集和存储逻辑。

代码已通过 Python 语法编译检查、UTF-8 中文自检、公式闭合检查和 Git 差异检查。静态约束检查确认颜色条不再使用固定 `90 px` 宽度，当前最小宽度为 `140 px`、最大宽度为 `180 px`；`HistogramLUTWidget.setImageItem()` 仍只在创建阶段调用一次；图像更新继续使用 `autoLevels=False`；直方图显示范围由 `_update_histogram_range()` 根据实际有限数据范围更新。当前 Python 环境未安装 `pyqtgraph`，因此无法执行 Qt 离屏颜色条运行验证，现场环境需要按专项文档中的验证场景继续检查视觉效果。
