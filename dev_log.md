...
本次将 Phase 显示弧度转换从 `float64` 改为 `float32` 原位乘法，以降低临时内存和主线程转换开销。Raw/IQ 保存改为保持原始 `int16` dtype，Phase 保持原始 `int32` dtype，避免 Raw 数据写盘时被扩展为双倍大小。Raw 文件大小估算也同步按每点两字节计算。
...
保存线程的磁盘空间耗尽处理进一步改为可恢复状态机。检测到 `ENOSPC` 或 Windows 磁盘空间不足错误后，保存工作线程保持运行，数据采集、图形显示和 TCP 发送不停止；保存器关闭故障文件、清空已有保存队列，并在暂停期间直接丢弃后续保存数据，避免大块数据在内存中积压。主界面会显示保存因磁盘满而暂停，而不是将其误报为整个采集任务停止。

保存工作线程每隔约 $2\ \mathrm{s}$ 尝试创建下一个编号文件并恢复写入。按照现场使用要求，磁盘完全写满但操作系统仍允许创建文件时，保存器保留这些连续编号的 `0 KB` 文件，并在下一次写入失败后继续尝试新的编号。释放磁盘空间后，新采集数据直接写入当前恢复文件并继续正常保存。磁盘满之前可能部分写入的故障文件不会继续追加，暂停期间数据计入 `dropped_blocks`，保证恢复行为不会阻塞采集线程，也不会掩盖数据缺口。
...
完整数据链路继续使用采集线程到保存队列的后台通路，GUI 仅消费最新显示快照。Phase 显示弧度转换改为 `float32` 原位乘法；慢循环告警阈值根据正常数据块周期动态计算；Raw/IQ 保存保持原始 `int16`，Phase 保存保持原始 `int32`。

磁盘写满后，数据采集、显示和 TCP 发送保持运行。保存线程暂停接收新保存数据并持续尝试创建递增编号文件；当操作系统允许创建文件但无法写入数据时，保留连续编号的 `0 KB` 文件。释放磁盘空间后，后续采集数据直接写入当前恢复文件并继续正常保存。暂停期间未写入的数据计入 `dropped_blocks`，防止保存队列无限增长。

### 文档与验证

本次更新同步维护了数据存储技术说明、Space-Time 与最新日志分析文档及开发日志。代码已执行 Python 语法编译检查、Git 差异检查、UTF-8 中文自检、磁盘满连续编号恢复模拟和暂停状态停止测试。模拟测试确认保存工作线程在磁盘满期间保持运行，恢复后能够继续写入；暂停状态下停止保存线程不会发生长时间阻塞。
