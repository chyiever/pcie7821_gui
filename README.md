...
把数据转换为弧度显示。但这个转换不回写保存链路，`FrameBasedFileSaver` 仍保存原始 `int32`。因此后续做离线工具或与其他系统对接时，不能把 GUI 上看到的单位直接当作磁盘中实际存储的单位。
...
当前主存储路径由 `FrameBasedFileSaver` 提供。保存时默认写二进制 `.bin` 文件，不写头、不写元数据索引、不做压缩。保存线程串行写入，必要时按 `frames_per_file` 分文件。当前代码的工程假设很明确：磁盘文件本身不是完整自描述格式，文件名和运行期参数日志共同构成离线解析所需的上下文。
