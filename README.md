...
采集线程每轮读取一个 `Length/Load` 完整块。完整块先进入后台消费者，包括保存和通信；显示链路只保留最新 `Length/Plot` 窗口。若 GUI 未及时消费旧快照，采集线程会跳过本次显示快照生成并记录 `gui_skips`，优先保证继续读卡和保存入队，避免 Qt 事件队列或显示快照拼接拖慢采集。
