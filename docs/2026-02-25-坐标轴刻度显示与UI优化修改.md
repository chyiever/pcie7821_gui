...
## 9. 【最新修复】坐标轴方向与刻度起始点问题

### 9.1 问题反馈（2026-02-25 下午）
用户反馈仍存在以下问题：
1. **滚动方向**: 还是向下滚动，而非向左滚动
2. **Y轴起始**: Y轴还是从0开始，而非distance_start
3. **时间轴影响**: time DS变化时，X轴刻度范围也会变化

### 9.2 根本解决方案

#### 核心修复1: 数据转置 (`src/time_space_plot.py:485`)
```python
...
```
**技术原理**:
- 原始数据 `(time_points, spatial_points)` 导致PyQtGraph默认Y轴为时间
- 转置后 `(spatial_points, time_points)` 使Y轴变为距离，X轴为时间
- 实现用户要求的坐标轴定义

#### 核心修复2: 坐标映射 (`src/time_space_plot.py:844-846`)
```python
...
```
**技术要点**:
- `setRect()`方法将图像像素坐标映射到真实坐标
- X轴范围: `[0, time_duration_s]` 秒
- Y轴范围: `[distance_start, distance_end]` 点数
- 解决Y轴从0开始的问题

#### 核心修复3: 时间计算独立性 (`src/time_space_plot.py:831`)
```python
# Time duration should be calculated from ORIGINAL frames, not downsampled
time_duration_s = original_time_points / scan_rate_hz
```
**关键改进**:
- 时间长度基于 `original_time_points`（原始帧数）
- 不受 `time_downsample` 参数影响
- 确保X轴时间范围稳定

#### 核心修复4: 视图范围控制 (`src/time_space_plot.py:874-877`)
```python
...
```
**强制控制**:
- 禁用自动范围调整 `enableAutoRange(enable=False)`
- 零填充 `padding=0` 确保紧贴坐标轴
- 强制视图范围匹配真实坐标

### 9.3 轴标签更新 (`src/time_space_plot.py:850-854`)
```python
# Update axis labels
...
### 9.5 验证测试文件
创建了 `test_coordinate_fixes.py` 综合验证文件：

#### 测试覆盖内容：
1. **滚动方向测试**: 创建向左移动的波形数据
2. **坐标映射测试**: 四角标记点验证坐标正确性
3. **时间轴独立性**: 不同Time DS设置下X轴范围不变
4. **Y轴起始点**: 验证从distance_start(40)开始
...
#### 问题分析
原有实现的根本问题是：
1. **数据方向错误**: 数据未转置，导致时间轴在垂直方向
2. **坐标映射缺失**: 缺少 `setRect()` 映射，导致默认从0开始
3. **时间计算耦合**: 时间长度计算受降采样影响

#### 解决策略
1. **数据流重组**: `time_space_data.T` 交换坐标轴
2. **坐标系重定义**: `setRect()` + `setRange()` 双重控制
3. **参数解耦**: 时间计算与降采样分离

#### 技术保证
- **版本兼容**: 使用PyQtGraph通用API，避免版本依赖
- **性能影响**: 转置操作O(1)复杂度，对性能影响极小
- **功能完整**: 所有原有功能保持不变
...
src/time_space_plot.py:
  + 数据转置逻辑 (line 485)
  + 导入QtCore模块 (line 26)
  + _update_axis_labels方法重写 (~65行)
  + 坐标映射实现 (setRect + setRange)
  + 时间计算独立性修复
...
#### 最终状态
- ✅ **滚动方向**: 向左滚动（X轴=时间）
- ✅ **Y轴起始**: 从distance_start开始
- ✅ **时间轴独立**: 不受Time DS影响
- ✅ **坐标映射**: 正确的真实坐标显示

---

**最终修复状态**: ✅ **Time-Space图坐标轴问题彻底解决**

**技术成就**: 通过数据转置、坐标映射和视图控制的组合方案，实现了用户要求的坐标轴定义，解决了滚动方向和刻度起始点的核心问题。