...
截至 2026-06-18 现场反馈，Phase 模式下 Tab1 在 `Mode=time` 与 `Mode=space` 之间切换后，波形已经可以直接显示，不再需要用户先画矩形放大，也不再需要手动点击 `Auto/View All`。该问题可以认为已经关闭。

本轮问题的最终结论如下：

1. 坐标公式本身是正确的。Raw、Phase-Time、Phase-Space 三类横轴分别使用距离轴或时间轴，公式保持如下：
   - Raw：$x=[1,2,\ldots,\mathrm{Points}] \times 0.1 \times \mathrm{DataRate}$。
   - Phase-Time：$x=[1,2,\ldots,N_{\mathrm{phase}}] \times 0.4 \times \mathrm{Rate2phase} \times \mathrm{Merge}$。
   - Phase-Space：$x=[1,2,\ldots,frame\_num]/\mathrm{Scan}$。
2. 问题根因不是 Phase-Space 数据为空，也不是 `space_data` 抽取失败，而是 Tab1 的 ViewBox 和曲线绘制状态在横轴物理单位切换后没有稳定恢复。
3. 最初的旧视图范围会在用户手动框选、滚轮缩放或平移后锁定。当横轴从米级距离轴切换到秒级时间轴时，如果没有主动恢复范围，新曲线会落在旧视图范围之外。
4. 只在模式切换瞬间调用 `_restore_plot_auto_range()` 不够稳定，因为当时新曲线尚未完成 `setData(x, y)`。
5. 只在 `setData(x, y)` 后调用一次自动范围仍不够稳定，因为 pyqtgraph 的 ViewBox 自动范围、PlotDataItem 边界缓存和 Qt 事件循环之间存在异步窗口。
6. 最终有效方案必须同时处理“范围恢复”和“曲线绘制路径刷新”。尤其是 `Mode=time -> Mode=space` 时，Space 曲线点数较少，不适合继续使用 Time 模式大曲线场景下的 `clipToView + auto downsampling` 优化。

最终有效实现由以下几部分共同组成：

- `_set_time_plot_axis()` 统一维护 Tab1 横轴标签和横轴类型，在 `distance` 与 `time` 切换时清空旧曲线并设置 pending 自动恢复标记。
- `_apply_pending_time_plot_auto_range()` 在新曲线完成 `setData(x, y)` 后恢复范围，并在后续若干帧继续尝试恢复，避免第一帧边界尚未稳定。
- `_curve_data_range()` 和 `_force_plot_range_to_curve_data()` 直接根据当前曲线真实 `x/y` 数据计算视图范围，不再只依赖 ViewBox 内部缓存。
- `_retry_time_plot_auto_range()` 通过 `QTimer.singleShot(0/50/150/300 ms)` 在 Qt 事件循环后重复恢复范围，覆盖曲线边界或绘制路径延迟更新的情况。
- `_configure_time_plot_curves_for_axis()` 在 Tab1 切到秒级 `time` 轴时关闭 `clipToView` 和自动降采样，切回 `distance` 轴时恢复原来的实时大曲线优化。
- `_refresh_plot_curve_items()` 在强制设置视图范围后主动刷新 PlotDataItem、PlotItem 和 ViewBox，避免旧裁剪路径保留空白显示。

后续维护时需要保留以下约束：

1. 不要把 Phase-Space 的 Tab1 曲线重新纳入 `clipToView + auto downsampling`，除非同时验证 `Mode=time -> Mode=space` 在手动缩放后仍能直接显示。
2. 不要删除 `setData(x, y)` 后的 pending 自动范围恢复；横轴单位切换必须在新数据写入后再恢复范围。
3. 不要只依赖 pyqtgraph 的 `autoRange()`；当前方案中按 `curve.getData()` 计算真实边界是避免 ViewBox 缓存不稳定的关键兜底。
4. 如果后续重构 Tab1 绘图控件或替换 ViewBox，需要重新验证以下现场路径：
   - Phase 模式，先在 `Mode=time` 中手动框选或滚轮缩放，再切换到 `Mode=space`。
   - Phase 模式，先在 `Mode=space` 中手动框选或滚轮缩放，再切换到 `Mode=time`。
   - 两个方向都应无需画矩形、无需点击 `Auto/View All`，波形应直接出现在新的横轴范围内。

本次最终修复不改变采集线程、保存线程、`FrameLoad`、`FramePlot`、Spectrum、Monitor 和 Tab2 Time-Space 图逻辑；影响范围仅限 Tab1 时域图在横轴物理单位切换后的视图恢复和曲线绘制刷新。
