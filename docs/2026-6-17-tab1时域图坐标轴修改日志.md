...
本次在 `src/main_window.py` 中新增 Tab1 横轴状态跟踪变量 `_time_plot_axis_kind`，并新增 `_set_time_plot_axis()` 方法统一处理横轴标签和横轴语义切换。

当横轴类型在 `distance` 与 `time` 之间切换时，程序会执行以下动作：

1. 更新 Tab1 底部标签，例如 `Distance (m)` 或 `Time (s)`。
2. 清空旧曲线，避免旧单位数据残留。
3. 对 `plot1` 调用 `_restore_plot_auto_range()`，解除缩放锁定并恢复 x/y 自动范围。
4. 记录日志 `Tab1 time plot axis changed`，便于后续定位模式切换行为。

`_on_mode_changed()` 中也同步调用 `_set_time_plot_axis()`，使用户点击 `Time` 或 `Space` 单选按钮时立即恢复合适的视图范围；随后下一帧数据进入 `_update_phase_display()` 时即可按新的时间轴正常显示。
...
flowchart TD
    ...
    E --> F[Enable plot1 auto range]
    F --> G[Next display data arrives]
    G --> H[setData with new x axis]
    H --> I[Waveform visible in new axis range]
```
